...
1. **`notionConstraints.json`** : Holds the theatrical improvisation constraints guide. This local cache is refreshed at build time or by a periodic task, by running the integration script:
   ...
   ```
2. **`reservoir-config.json`** : The prompt reservoir used by the offline generators. It is regenerated automatically **every Saturday at 06:00 UTC** by the [refresh-pool.yml](file:///c:/Projects/eole.me/improv-assist/.github/workflows/refresh-pool.yml) workflow, which queries `/api/improv-regen` one category at a time (25 seconds apart, that route allowing 3 requests per minute) and commits the result to `main`: one generation a week now serves every client, where each user used to pay for their own. Each request carries the items already in that category as `avoid`, without which the model converges on the same answer: the first two runs reproduced four categories out of ten item-for-item, and the log now prints a novelty count per category so that stops being an assumption. A category that comes back empty, truncated below half its size, or in error keeps the items already shipped. The new pool reaches production at the next `make deploy`.
   ```bash
